    ...
    public static <V> HashMap<V, Integer> Dijkstra(DiGraph<V> G, V source, V dst)
    {
      ...
      for (V vertex : G.vertices()) {
        ...
        if(vertex.equals(source)){
          ...
          priq.add(new Edge<V, Integer>(vertex, 0));        
        }
          else{
          dist.put(vertex, Integer.MAX_VALUE);
          pred.put(vertex, null);  
          priq.add(new Edge<V, Integer>(vertex, Integer.MAX_VALUE));
          }
      }
      ...
      while (!priq.isEmpty()) {
        ...
        // for(int i=0; i < numVert; i++){
          Edge<V, Integer> currentEdge = priq.removeMin();
          V currentVertex = currentEdge.getVertex();
          
          if(currentEdge.getCost() == Integer.MAX_VALUE){
             break;
          }

          // Process each adjacent vertex of the current vertex
          for (Edge<V, Integer> neighborEdge : G.adjacentTo(currentVertex)) {
              V neighbor = neighborEdge.getVertex();
              int weight = neighborEdge.getCost();
              
              // Calculate the new distance through the current vertex
              int newDistance = dist.get(currentVertex) + weight;
              
              // If the new distance is shorter, update dist and pred maps
              if (newDistance < dist.get(neighbor)) {
                  dist.put(neighbor, newDistance);
                  pred.put(neighbor, currentVertex);
                  priq.add(new Edge<V, Integer>(neighbor, newDistance));
              }
          }
      }

      // Reconstruct the path from source to destination using pred map
